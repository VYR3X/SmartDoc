...
protocol OperationHistoryServiceProtocol {

	func getDoctorTickets(resourceID: String,
						  completion: @escaping (Result<OperationHistoryModel, Error>) -> Void)

}

/// Интерактор списка специальностей врачей
class OperationHistoryService: OperationHistoryServiceProtocol {
	...
	func getDoctorTickets(resourceID: String,
		...
		let resource = "SLOT"
		let method = "SEARCH"
		let params = "{RESOURCE_ID:\"\(resourceID)\"}"
		...
		session.dataTask(with: url!) { (data , responce, error) in
			...
			if let responce = responce { print(responce) }
			...
			do {
				let slots = try JSONDecoder().decode(OperationHistoryModel.self, from: win1251Data)
